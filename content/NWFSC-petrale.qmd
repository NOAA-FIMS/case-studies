...
# # reading from Ian's computer to create RDS file
# petrale_output <- r4ss::SS_output("c:/SS/petrale/Petrale2023/petrale/models/2023.a034.001/")
# saveRDS(petrale_output, file.path("data_files", "petrale_output_from_SS3.RDS"))

# load output from SS3 model (RDS file created using commented-out commands above)
petrale_output <- readRDS(file.path("data_files", "petrale_output_from_SS3.RDS"))
...
mydat <- get_ss3_data(ss3dat, fleets = c(1,4), ages = ages, lengths = petrale_input$dat$lbin_vector)
...
init_naa <- rep$natage |> 
  ...
  as.numeric() #*1000 # TODO: figure out if *1000 is needed
...
parameters <- default_parameters |>
  ...
  update_parameters(
    modified_parameters = list(
      ...
      maturity = list(
        LogisticMaturity.inflection_point.value = 6.5, 
        LogisticMaturity.inflection_point.estimated = FALSE,
        LogisticMaturity.slope.value = 2, # arbitrary guess
  ...
  update_parameters(
    modified_parameters = list(
      population = list(
        ...
        # TODO: changing this to TRUE resulted in worse estimates and warning:
        #   Warning messages:
        #   1: In sqrt(diag(cov)) : NaNs produced
        #   2: In sqrt(diag(object$cov.fixed)) : NaNs produced
        Population.log_M.value = rep(log(rep$parameters["NatM_uniform_Fem_GP_1", "Value"]), 
...
dplyr::filter(Yr %in% years) |> 
dplyr::rename("SS3_SpawnBio" = "SpawnBio",
              "SS3_Bio" = "Bio_all") |>
dplyr::mutate(FIMS_SpawnBio = get_report(fit)[["ssb"]][[1]][-1] , 
              FIMS_Bio = get_report(fit)[["biomass"]][[1]][-1]) |> ##CHECK: Is FIMS ssb reporting nyears+1 or initial year-1?
