...
stock_synthesis_data <- r4ss::ss3_data_to_fims(
  ...
  dplyr::mutate(
    uncertainty = ifelse(
      test = type == "age" & fleet == "Acoustic_Survey",
      uncertainty * 30,
      uncertainty
    )
  )
...
parameters <- FIMS::setup_default_parameters(data = data_4_model) |>
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      timing = dplyr::pull(natural_mortality_matrix, year),
      age = dplyr::pull(natural_mortality_matrix, age)
    ),
    by = c("module_name", "label", "timing", "age")
  ) |>
  dplyr::rows_update(
    tibble::tibble(
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      fleet = rep(c("Fishery", "Acoustic_Survey"), each = 2),
...
fleets <- get_fleets(data_4_model)
...
for (i in seq_along(fleets)) {
  ...
  if (i == 1 & time_varying_selectivity) {
    selectivity_type <- parameters |>
      dplyr::filter(module_name == "Selectivity", fleet == "Fishery") |>
      dplyr::slice(1) |>
  ...
  if ("catch" %in% fleet_types) {
    ...
    fleet_landings[[i]] <- methods::new(
      Catch,
      get_n_years(data_4_model)
    )
    fleet_landings[[i]]$catch_data[] <- model_catch(
      data_4_model,
      fleet = fleets[i]
    )
    ...
    fleet_module_ids <- c(
      fleet_module_ids,
      c(catch = fleet_landings[[i]]$get_id())
    )
  }
  ...
  if ("index" %in% fleet_types) {
    ...
    fleet_index[[i]] <- methods::new(
      Index,
      get_n_years(data_4_model)
    )
    fleet_index[[i]]$index_data[] <- model_index(
      data_4_model,
      fleet = fleets[i]
    )
    fleet_module_ids <- c(
  ...
  if ("age_comp" %in% fleet_types) {
    ...
    fleet_age_comp[[i]] <- methods::new(
      AgeComp,
      get_n_years(data_4_model),
      get_n_ages(data_4_model)
    )
    fleet_age_comp[[i]]$age_comp_data[] <- model_age_comp(
      data_4_model,
      fleet = fleets[i]
    )
    fleet_module_ids <- c(
  ...
  if ("index" %in% fleet_types) {
    fleet_index_distribution[[i]] <- FIMS:::initialize_data_distribution(
      module = fleet[[i]],
      data_type = "index",
      uncertainty = get_data(data_4_model) |>
        dplyr::filter(fleet == fleets[i] & type == "index") |>
  ...
  if ("catch" %in% fleet_types) {
    fleet_landings_distribution[[i]] <- FIMS:::initialize_data_distribution(
      module = fleet[[i]],
      uncertainty = get_data(data_4_model) |>
        dplyr::filter(fleet == fleets[i] & type == "catch") |>
        dplyr::pull(uncertainty),
      data_type = "catch"
    )
  }
  ...
  if ("age_comp" %in% fleet_types) {
    fleet_agecomp_distribution[[i]] <- FIMS:::initialize_data_distribution(
      ...
      data_type = "age_comp",
      uncertainty = get_data(data_4_model) |>
        dplyr::filter(fleet == fleets[i] & type == "age_comp") |>
        dplyr::pull(uncertainty)
    )
  }
}
...
recruitment$log_devs$set_estimation_types(c("constant"))
  ...
  dplyr::left_join(
    colnames(posterior) |>
      ...
      dplyr::mutate(
        ...
        fleet = dplyr::if_else(fleet_number == 1, "Fishery", "Acoustic_Survey"),
        words = gsub("_", " ", label)
      ) |>
      dplyr::ungroup(),
    by = c("name" = "value")
...
ggplot2::ggplot(
  ...
  ggplot2::facet_wrap(fleet ~ words, scales = "free_x") +
