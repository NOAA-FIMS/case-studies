...
```{r}
#| label: plot-selectivity-fishery
#| fig-cap: "Posterior estimates of the logistic selectivity curves for the fishery."
#| fig-alt: "The slope of the fishery curve was poorly estimated."
#| echo: false
#| warning: false
#| eval: !expr time_varying_selectivity

ggplot2::ggplot(
  posteriors_labeled |>
    dplyr::filter(module_name == "Selectivity", fleet_number == 1),
  ggplot2::aes(x = value)
) +
  tidybayes::stat_slabinterval(
    fill = "skyblue", 
...
```{r}
#| label: plot-selectivity-fishery-inflection-point-zoom
#| fig-cap: "Posterior estimates of the inflection point (x-axis; age) of the logistic curve for time-varying fishery selectivity."
#| fig-alt: "Over the last decade we have been selecting younger fish."
#| echo: false
#| warning: false
...
ggplot2::ggplot(
  posteriors_labeled |>
    dplyr::filter(
      ...
      label == "inflection_point",
      fleet_number == 1,
      year > get_end_year(data_4_model) - 2
    ),
  ggplot2::aes(x = value, y = as.factor(year), fill = ggplot2::after_stat(x))
) +
  ggridges::geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
...
```{r}
#| label: plot-log-Fmort-recent
#| fig-cap: "Posterior estimates of the natural log of fishing mortality for recent years (y axis)."
