...
ages <- 0:15
source(here::here("content", function_directory, "get_ss3_data.R"))
```
...
# NOTE: Cannot change this instance of file.path() to here::here() because it is using
#       files from the web not files from this local directory
...
#TODO: save this figure and reference it above with maturity
maturity_data |>
...
# index
get_estimates(fit) |>
  dplyr::filter(label == "IndexExpected") |>
  # TODO: fix this filtering when module_id has fleet number in it again
  dplyr::mutate(
    module_id = rep(
      seq(get_n_fleets(fims_data)),
      each = length(get_start_year(fims_data):get_end_year(fims_data))
    ),
    year = rep(
      get_start_year(fims_data):get_end_year(fims_data),
      get_n_fleets(fims_data)
    )
  ) |>
  dplyr::filter(module_id == 2) |>
  ggplot2::ggplot(
    ggplot2::aes(
      year,
      estimate
...
#| echo: false
#| eval: false
#| label: comparison-plots-not-run
