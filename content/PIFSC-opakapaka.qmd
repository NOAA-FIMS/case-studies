...
opaka_dat_fims <- opaka_dat_fims |>
  dplyr::mutate(name = ifelse(name == "FRS" & type == "index", "FRS_CPUE", name))

# Set sample size for length compositions to 1.0.
opaka_dat_fims <- opaka_dat_fims |>
  dplyr::mutate(uncertainty = ifelse(type == "length_comp" & name == "BFISH", 1, uncertainty)) #|>
...
init_naa <- (exp(opaka_inputs_bootstrap$ctl$SR_parms["SR_LN(R0)", "INIT"]) * 1000) * exp(-(ages - 1) * 0.135)
...
index_results <- data.frame(
  observed = FIMS::model_index(data_4_model, "BFISH"),
...
cpue_results <- data.frame(
  observed = FIMS::model_index(data_4_model, "FRS_CPUE"),
  expected = FIMS::get_report(fit)[["index_expected"]][[2]]
) |>
  dplyr::mutate(year = years) |>
...
catch_results <- data.frame(
  ...
  expected = c(
    FIMS::get_report(fit)[["landings_expected"]][[1]],
    FIMS::get_report(fit)[["landings_expected"]][[3]]
  ),
