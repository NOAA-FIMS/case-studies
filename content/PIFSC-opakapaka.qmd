...
default_parameters <- FIMS::create_default_configurations(
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      value = c(opaka_inputs_bootstrap$ctl$SR_parms["SR_LN(R0)", "INIT"], -log(1.0 - 0.76) + log(0.76 - 0.2), opaka_inputs_bootstrap$ctl$SR_parms["SR_sigmaR", "INIT"])
    ),
    by = c("module_name", "label")
  ) |>
  dplyr::rows_update(
...
catch_results <- data.frame(
  observed = c(
    FIMS::m_landings(data_4_model, fleet = "fleet1"),
    FIMS::m_landings(data_4_model, fleet = "fleet3")
  ),
  expected = c(
