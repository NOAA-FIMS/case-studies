  ...
  update_parameters(
    modified_parameters = list(
      fleet1 = list(
        ...
        # KFJ: removed the log
        # FIX: old code did not estimate observation error around the F
        DlnormDistribution.log_sd.value = rep(sqrt(log(as.numeric(mean(rdat$control.parms$catch.tot.cv[,1], na.rm=TRUE)^2) + 1)), nyears)
        # FIX: The below code doesn't match what is up above
        # DlnormDistribution.log_sd.value = get_data(age_frame) |>
        #   dplyr::filter(name == "fleet1", type == "landings") |>
        #   dplyr::pull(uncertainty)
      ),
      survey1 = list(
        LogisticSelectivity.inflection_point.value = rdat$sel.input.mats$index.sel.ini[(1+1)*(nages+2+4)+nages+1,1], # hardwired for this example
        ...
        #      the true slope value from
        LogisticSelectivity.slope.value = rdat$sel.input.mats$index.sel.ini[(1+1)*(nages+2+4)+nages+1,1],
      ...
      survey2 = list(
        LogisticSelectivity.inflection_point.value = rdat$sel.input.mats$index.sel.ini[(2+1)*(nages+2+4)+nages+1,1], # hardwired for this example
        # FIX: A bug in previous example where inflection.point was set twice rather
        #      than setting slope, copy and paste error so I am not sure where to get
        #      the true slope value from
        LogisticSelectivity.slope.value = rdat$sel.input.mats$index.sel.ini[(1+2)*(nages+2+4)+nages+1,1],
