...
parameters <- setup_default_parameters(data = data_4_model) |>
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      timing = rep(years, each = get_n_ages(data_4_model)),
    ...
    by = c("module_name", "label", "age", "timing"),
    # TODO: fix this
    unmatched = "ignore"
  ) |>
...
convert_output_asap <- function(report, fleets) {
  ...
  dplyr::bind_rows(
    ...
    dplyr::mutate(
      ...
      fleet = as.character(factor(module_id, labels = fleets)),
      era = "time"
    )
}
...
catch_results <- data.frame(
  observed = get_data(data_4_model) |>
    dplyr::filter(type == "catch", fleet == "fishery") |>
    dplyr::pull(observed),
  FIMS = report$catch_expected[[1]],
