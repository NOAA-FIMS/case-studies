    ...
    modified_parameters = list(
      recruitment = list(
       ...
       # BevertonHoltRecruitment.estimate_log_devs = FALSE, #CHECK what is the difference between this and log_devs.estimated? 
        ...
        #DnormDistribution.log_sd.value = (opaka_ctl$SR_parms["SR_sigmaR", "INIT"])
      )
    )
  ) |>
  update_parameters(
    modified_parameters = list(
      maturity = list(
        LogisticMaturity.inflection_point.value = 7, #CHECK: does this need to be in terms of age? currently in opaka model in terms of length so guesstimated what it would be for age based on growth
...
fit@estimates |> filter(name == "log_rzero")
pull(value) #negative values for init_naa??
