...
parameters <- default_parameters |>
  ...
  update_parameters(
    modified_parameters = list(
      recruitment = list(
        BevertonHoltRecruitment.log_rzero.value = opaka_ctl$SR_parms["SR_LN(R0)", "INIT"], #log(exp(opaka_ctl$SR_parms["SR_LN(R0)", "INIT"])*1000), #to deal with SS reporting R0 in thousands (from Ian's case study)
        #BevertonHoltRecruitment.log_rzero.estimated = FALSE,
        BevertonHoltRecruitment.log_devs.value = recdevs$Value,
        BevertonHoltRecruitment.log_devs.estimated = FALSE,
       # BevertonHoltRecruitment.estimate_log_devs = FALSE, #CHECK what is the difference between this and log_devs.estimated? 
  ...
  update_parameters(
    modified_parameters = list(
      maturity = list(
        LogisticMaturity.inflection_point.value = 4.5, #CHECK: does this need to be in terms of age? currently in opaka model in terms of length so guesstimated what it would be for age based on growth
