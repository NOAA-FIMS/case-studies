...
acomps <- read.csv("data_files/sardine_acomps.csv") %>%
  mutate(value_prop = value / Nsamp) # convert age-comp data to proportions, to match fims-demo.Rmd
...
#                               age = 0:8, 
#                               yr = 2005:2023)
# blank_age <- tibble(type = "age", name = blank_age_grid$name,
...
#-----Weight-at-age
  # Q: What are the differences in how SS3 and FIMS process wtatage inputs/units?
    # SS3 assumes WAA is expressed in kg, and then converts to mt for biomass calculations (presumably)   
    # FIMS assumes WAA is expressed in mt
    # The same WAA input values (in kg) are supplied to both SS3 and FIMS
    # Despite this, model output shows similar summary biomass between SS3 and FIMS, with similar naa and waa values
wtatage <- r4ss::SS_readwtatage("data_files/sardine_wtatage.ss_new") %>%
  filter(fleet %in% c(1, 2)) %>%
  mutate(fleet = ifelse(fleet == 1, "fleet1", "survey1")) %>%
  pivot_longer(cols = `0`:`10`, names_to = "age", values_to = "value") %>%
  filter(year != 2024, !(age %in% c("9", "10"))) # Trim ages 9 and 10 to match age-comps
...
nyears <- final_fimsdat@n_years
years <- final_fimsdat@start_year:final_fimsdat@end_year
nages <- final_fimsdat@n_ages
ages <- final_fimsdat@ages
...
ssbs <- ssres$timeseries %>% select(Yr, SpawnBio) %>% 
  mutate(fims = c(0, 0, get_report(fit)[["ssb"]][[1]]))
names(ssbs)[2] <- 'ss3'
...
waa <- data.frame(age = wtatage$age[(wtatage$year == 2005 & wtatage$fleet == "fleet1")], 
...
recs <- ssres$timeseries %>% select(Yr, Recruit_0) %>%
  mutate(fims = c(0, 0, get_report(fit)[["recruitment"]][[1]]))
names(recs)[2] <- "ss3"
