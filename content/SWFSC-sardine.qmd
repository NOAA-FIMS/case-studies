...
-   TMB version: `r packageDescription("TMB")$Version`\
-   FIMS commit: `r substr(packageDescription("FIMS")$GithubSHA1, 1, 7)`\
...
ssbs %>% filter(Yr >= 2005, Yr < 2024) %>% melt(id.var = "Yr") %>%
  ...
  geom_point() + geom_line() + scale_y_continuous(label = comma)
...
age1plus <- age1plus %>% left_join(bio1)
