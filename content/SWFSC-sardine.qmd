...
fimscatch <- tibble::tibble(type = "catch", fleet = "fleet1",
                    age = NA, timing = catch$year, observed = catch$catch,
                    unit = "mt",
                    uncertainty = paste0(
                      "~dlnorm(meanlog = log_catch_expected, sdlog = 0.00001",
                      # sqrt(log(0.01^2 + 1)), # Input was 0.05 but log(sqrt(log(0.01^2 + 1)))
                                             # was used in parameter tibble
                      ")"
                    ))
#-----CPUE
...
fimsindex <- tibble::tibble(type = "index", fleet = "survey1",
                    age = NA, timing = cpue$year,
                    observed = cpue$obs, unit = "mt",
                    uncertainty = paste0(
                      "~dlnorm(meanlog = log_catch_expected, sdlog = ",
                      sqrt(log(0.01^2 + 1)), # Input was 0.3 but log(sqrt(log(0.01^2 + 1)))
                                             # was used in parameter tibble
                      ")"
                    )
)
...
acomps <- utils::read.csv(file.path(data_directory, "sardine_acomps.csv")) |>
  ...
  # convert value column to proportions, to match fims-demo.Rmd
  dplyr::mutate(value_prop = value / sum(value)) |>
  dplyr::ungroup()


fimsage <- tibble::tibble(type = "age_comp", fleet = acomps$name,
                  age = acomps$age, timing = acomps$Yr,
                  observed = acomps$value_prop, unit = "proportion",
                  uncertainty = paste0("~dmultinom(prob = agecomp_proportion, size = ", acomps$Nsamp, ")"))
...
wtatage <- r4ss::SS_readwtatage(file.path(data_directory, "sardine_wtatage.ss_new")) |>
  ...
  dplyr::filter(!(age %in% c("9", "10"))) |> # Trim ages 9 and 10 to match age-comps
...
data_4_model$observed <- as.numeric(data_4_model$observed)
...
rzero <- 14.2 + log(1000) #14.2 is log(R0) in sardine simplified model
...
parameters <- FIMS::setup_default_parameters(data = data_4_model) |>
  dplyr::rows_update(
    tibble::tibble(
      ...
      fleet = rep(c("fleet1", "survey1"), each = 2)
    ),
    by = c("module_name", "label", "fleet")
  ) |>
  dplyr::rows_update(
    tibble::tibble(
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      value = c(1.2, 2.5) # 1.5 is arbitrary guess
    ),
    by = c("module_name", "label")
  ) |>
  dplyr::rows_update(
