...
#-----Age compositions
acomps <- utils::read.csv(here::here("content", data_directory, "sardine_acomps.csv")) |>
...
wtatage <- r4ss::SS_readwtatage(here::here("content", data_directory, "sardine_wtatage.ss_new")) |>
...
estimate_fish_selex <- "constant"
estimate_survey_selex <- "constant"
estimate_q <- "constant" #Fix at 1
estimate_F <- "fixed_effects"
estimate_recdevs <- "fixed_effects"
estimate_init_naa <- "fixed_effects"
estimate_log_rzero <- "fixed_effects"
...
n_years <- get_n_years(final_fimsdat)
years <- get_start_year(final_fimsdat):get_end_year(final_fimsdat)
n_ages <- get_n_ages(final_fimsdat)
...
parameters <- default_parameters |>
  update_parameters( # update fleet1 specifications
    modified_parameters = list(
      fleet1 = list(
        ...
        LogisticSelectivity.inflection_point.estimation_type = estimate_fish_selex,
        LogisticSelectivity.slope.value = 5,
        LogisticSelectivity.slope.estimation_type = estimate_fish_selex,
        Fleet.log_Fmort.value = log(rep(0.2, n_years)),
        Fleet.log_Fmort.estimation_type = estimate_F,
        DlnormDistribution.log_sd.value = rep(log(sqrt(log(0.01^2 + 1))), n_years)
      )
    )
  ) |>
  update_parameters( # update survey1 specifications
    modified_parameters = list(
      survey1 = list(
        ...
        LogisticSelectivity.inflection_point.estimation_type = estimate_survey_selex,
        LogisticSelectivity.slope.value = 2,
        LogisticSelectivity.slope.estimation_type = estimate_survey_selex,
        Fleet.log_q.value = 0,
        Fleet.log_q.estimation_type = estimate_q,
        DlnormDistribution.log_sd.value = rep(log(sqrt(log(0.1^2 + 1))), n_years)
      )
    )
  ) |>
  update_parameters(
    modified_parameters = list(
      recruitment = list(
        ...
        BevertonHoltRecruitment.log_rzero.estimation_type = estimate_log_rzero,
        BevertonHoltRecruitment.logit_steep.value = -log(1.0 - steep) + log(steep - 0.2),
        BevertonHoltRecruitment.log_devs.value = rep(log(1), n_years-1),
  ...
  update_parameters(
    modified_parameters = list(
      population = list(
        Population.log_M.value = rep(log(M_value), n_ages * n_years),
        Population.log_init_naa.value = log(init_naa),
        Population.log_init_naa.estimation_type = estimate_init_naa
      )
    )
  )
```
...
load(here::here("content", data_directory, "sardine_simplified_res.Rdata"))
