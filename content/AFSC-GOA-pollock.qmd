...
default_parameters <- FIMS::create_default_configurations(
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      fleet = c("fleet1", "survey2", "survey6"), #previously fleet_name
      module_type = "DoubleLogistic"
    ),
    by = c("module_name", "fleet") #previously fleet_name
  ) |>
  ...
  dplyr::rows_update(
    tibble::tibble(
      module_name = "Selectivity",
      fleet = "fleet1", #previously fleet_name
      label = c(
    ...
    by = c("module_name", "fleet", "label") #previously fleet_name
  ) |>
  dplyr::rows_update(
    tibble::tibble(
      module_name = "Selectivity",
      fleet = "survey2", #previously fleet_name
      label = c(
        "inflection_point_asc", "slope_asc",
        "inflection_point_desc", "slope_desc"
      ),
  ...
  dplyr::rows_update(
    tibble::tibble(
      module_name = "Selectivity",
      fleet = "survey6", #previously fleet_name
      label = c(
        "inflection_point_asc", "slope_asc",
        "inflection_point_desc", "slope_desc"
      ),
  ...
  dplyr::rows_update(
    tibble::tibble(
      module_name = "Selectivity",
      fleet = "survey3", #previously fleet_name
  ...
  dplyr::rows_update(
    ...
    by = c("module_name", "fleet", "label", "time") #previously fleet_name
  ) |>
  dplyr::rows_update(
    tibble::tibble(
      module_name = "Fleet",
      fleet = c("survey2", "survey3", "survey6"), #previously fleet_name
      label = "log_q",
...
```{r}
#| label: comparison-plots-raw
#| fig-cap: "Comparisons of biomass, catch, fishing mortality (F), survey indices of abundance, recruitment, and spawning biomass between the Template Model Builder (TMB) model and the optimized FIMS model."
#| fig-alt: "All metrics match quite well between the two models."
#| echo: false
#| warning: false
out1 <- get_long_outputs(rep1, rep0, output) |>
  dplyr::mutate(platform = ifelse(platform == "FIMS", "FIMS init", "TMB"))
out2 <- get_long_outputs(rep2, rep0, output) |>
  dplyr::filter(platform == "FIMS") |>
  dplyr::mutate(platform = "FIMS opt")
out <- rbind(out1, out2)
g <- ggplot2::ggplot(
  out |>
    dplyr::filter(platform != "FIMS init"),
...
```{r}
#| label: comparison-plots-re
#| fig-cap: "Relative error in biomass, catch, fishing mortality (F), survey indices of abundance, recruitment, and spawning biomass between the Template Model Builder (TMB) model and the optimized FIMS model. Relative error is also shown for the pre-optimized FIMS models (FIMS init)."
#| fig-alt: "All metrics match quite well between the two models."
#| echo: false
#| warning: false
g <- ggplot2::ggplot(
...
```{r}
#| label: comparison-plots-age-comp
#| fig-cap: "Proportion at age (paa) by year (panel) for the fishing fleet for the initialized FIMS model (FIMS init), the optimized FIMS model (FIMS opt), the Template Model Builder model (TMB), and observed data (obs)."
#| fig-alt: "Older ages are overestimated in 2010 by all models."
#| echo: false
#| warning: false
...
```{r}
#| label: comparison-plots-age-comp-survey
#| fig-cap: "Proportion at age (paa) by year (panel) for the first survey for the initialized FIMS model (FIMS init), the optimized FIMS model (FIMS opt), the Template Model Builder model (TMB), and observed data (obs)."
#| fig-alt: "The best fits are in year 2001."
#| echo: false
#| warning: false
...
```{r}
#| label: comparison-plots-index
#| fig-cap: "Fits to the indices of abundance (million metric tons) by survey (panel) for the initialized FIMS model (FIMS init), the optimized FIMS model (FIMS opt), the Template Model Builder model (TMB). Observed data with uncertainty measurements are plotted using points."
#| fig-alt: "All models fit the same."
#| echo: false
#| warning: false
