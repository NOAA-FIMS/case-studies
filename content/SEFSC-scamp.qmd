...
sca <- dget(here::here("content", data_directory, "/scamp32o.rdat"))
...
fleet1_ac_df <- data.frame(
  ...
  datestart = rep(paste0(seq(styr, endyr), "-01-01"), each = n_ages),
  dateend = rep(paste0(seq(styr, endyr), "-12-31"), each = n_ages),
...
weight_at_age <- data.frame(
  ...
  name = "fleet1",
  age = seq(n_ages),
  datestart = rep(paste0(styr, "-01-01"), n_ages),
  dateend = rep(paste0(styr, "-12-31"), n_ages),
...
# FIX
# $Population.log_M.estimatation_type <- "fixed_effects"
# [1] 1
...
fims.naa <- matrix(report$naa[[1]], ncol = n_ages, byrow = TRUE)
fims.naa <- fims.naa[-54, ]
popn_results <- data.frame(
  yr = styr:endyr,
  fims.ssb = report$ssb[[1]][1:n_years],
  fims.recruits = report$recruitment[[1]][1:n_years] / 1000,
...
fims.fleet1.ncaa <- matrix(report$landings_naa[[1]], ncol = n_ages, byrow = TRUE)
...
fims.fleet2.ncaa <- matrix(report$landings_naa[[2]], ncol = n_ages, byrow = TRUE)
...
fims.survey.ncaa <- matrix(report$landings_naa[[3]], ncol = n_ages, byrow = TRUE)
