...
default_parameters <- FIMS::create_default_configurations(data_4_model) |>
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      label = c("log_M"),
      time = rep(seq(get_start_year(data_4_model), get_end_year(data_4_model)), each = n_ages),
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      fleet = "survey1",
      # time = years,
      time = seq(get_start_year(data_4_model), get_end_year(data_4_model)),
      label = "log_Fmort",
...
fleet1_landings_results <- data.frame(
  ...
  observed = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    FIMS::get_data(data_4_model) |>
      dplyr::filter(type == "landings", fleet == "fleet1") |>
      dplyr::pull(value)
  ),
  fims.expected = c(rep(NA, length(styr:get_start_year(data_4_model)) - 1), report$landings_expected[[1]]),
...
fleet2_landings_results <- data.frame(
  ...
  observed = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    FIMS::get_data(data_4_model) |>
      dplyr::filter(type == "landings", fleet == "fleet2") |>
      dplyr::pull(value)
  ),
  fims.expected = c(rep(NA, length(styr:get_start_year(data_4_model)) - 1), report$landings_expected[[2]]),
...
plot(fleet1_landings_results$yr, fleet1_landings_results$observed,
  ylim = c(0, max(fleet1_landings_results[, -1], na.rm = TRUE)),
...
plot(fleet2_landings_results$yr, fleet2_landings_results$observed,
  ylim = c(0, max(fleet2_landings_results[, -1], na.rm = TRUE)),
