...
The model presented in this case study was changed substantially from the operational version and should not be considered reflective of the `r common_name` stock. These results are intended to be a demonstration for comparing the results of a FIMS model to the results of a Beaufort Assessment Model (BAM).

To get the operational model to more closely match a FIMS model the following changes were made:

The original stock assessment ([SEDAR-68OA](https://sedarweb.org/documents/sedar-68oa-south-atlantic-scamp-operational-assessment-final-stock-assessment-report/)) was conducted using the Beaufort Assessment Model (BAM). That assessment included options not yet available in FIMS. So, the following simplifications or modifications were made to the BAM configuration to allow more direct comparisons with FIMS output.

* Set spawning biomass calculations to occur on January 1, rather than the time of peak spawning.
* Set abundance calculations for matching index data to occur on January 1, rather than mid-year sampling.
...
* Converted spawning biomass to be female only. Because scamp are a protogynous hermaphrodite, the original assessment accounted for males implicitly by computing spawning biomass as the sum of total (male + female) mature biomass. One way to match that accounting would be to assume that the female maturity vector equals that of both sexes combined, and then further assume that the population is 100% female. However, current FIMS does not allow deviation from a 50:50 sex ratio. Thus, for this example, FIMS and BAM use the maturity vector of both sexes combined, but apply that vector only to females and assume a 50:50 sex ratio when computing spawning biomass.
* Female maturity at age modeled as a logistic function, rather than empirical.
* Dropped the fishery-dependent growth curve and set mean size at age of the catch equal to the population growth curve.
...
* Converted all observed and predicted catch to weight (mt). The original assessment used units native to the data collection: commercial in pounds and recreational in numbers.
* Dropped fishery-dependent indices.
...
fleet1_catch <- sca$t.series$L.COM.ob
fleet1_catch_cv <- sca$t.series$cv.L.COM
fleet1_catch_sd <- sqrt(log(1.0 + fleet1_catch_cv^2))
fleet2_catch <- sca$t.series$L.REC.ob
fleet2_catch_cv <- sca$t.series$cv.L.REC
fleet2_catch_sd <- sqrt(log(1.0 + fleet2_catch_cv^2))
...
fleet1_catch_df <- data.frame(
  type = "catch",
  fleet = "fleet1",
  age = NA,
  timing = seq(styr, endyr),
  observed = as.numeric(fleet1_catch),
  unit = "mt",
  uncertainty = paste0(
    "~dlnorm(meanlog = log_catch_expected, sdlog =0.0001",
    # fleet1_catch_sd,
    ")"
  )
)

fleet2_catch_df <- data.frame(
  type = "catch",
  fleet = "fleet2",
  age = NA,
  timing = seq(styr, endyr),
  observed = as.numeric(fleet2_catch),
  unit = "mt",
  uncertainty = paste0(
    "~dlnorm(meanlog = log_catch_expected, sdlog =0.0001",
    # fleet2_catch_sd,
...
survey_index_df <- data.frame(
  ...
  age = NA,
  timing = seq(styr, endyr),
  observed = as.numeric(survey_index),
  unit = "",
  uncertainty = paste0(
    "~dlnorm(meanlog = log_catch_expected, sdlog =0.01",
    # survey_index_sd,
    ")"
  )
)
...
fleet1_ac_df <- data.frame(
  ...
  observed = as.numeric(t(fleet1_ac)),
  unit = "number",
  uncertainty = paste0(
    "~dmultinom(prob = agecomp_proportion, size = ",
    rep(fleet1_ac_n, each = n_ages),
    ")"
  )
)
...
fleet2_ac_df <- data.frame(
  ...
  age = rep(seq(1, n_ages), n_years),
  timing = rep(seq(styr, endyr), each = n_ages),
  observed = as.numeric(t(fleet2_ac)),
  unit = "number",
  uncertainty = paste0(
    "~dmultinom(prob = agecomp_proportion, size = ",
    rep(fleet2_ac_n, each = n_ages),
    ")"
  )
)
...
survey_ac_df <- data.frame(
  ...
  age = rep(seq(1, n_ages), n_years),
  timing = rep(seq(styr, endyr), each = n_ages),
  observed = as.numeric(t(survey_ac)),
  unit = "number",
  uncertainty = paste0(
    "~dmultinom(prob = agecomp_proportion, size = ",
    rep(survey_ac_n, each = n_ages),
    ")"
  )
)
...
default_parameters <- FIMS::setup_default_parameters(data_4_model) |>
  # Maturity
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      fleet = rep(c("fleet1", "fleet2", "survey1"), each = 2),
      label = rep(c("slope", "inflection_point"), 3),
      value = c(
        sca$parm.cons$selpar_slope_COM2[8],
        sca$parm.cons$selpar_A50_COM2[8],
        sca$parm.cons$selpar_slope1_REC2[8],
        sca$parm.cons$selpar_A50_REC2[8],
        sca$parm.cons$selpar_slope1_CVT[8],
        sca$parm.cons$selpar_A501_CVT[8]
      ),
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      label = c("log_M"),
      timing = rep(seq(get_start_year(data_4_model), get_end_year(data_4_model)), each = n_ages),
      age = rep(ages, get_n_years(data_4_model)),
      value = log(as.numeric(t(matrix(
        rep(sca$a.series$M, each = get_n_years(data_4_model)),
        nrow = get_n_years(data_4_model)
      )))),
      estimation_type = "constant"
    ),
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      value = c(
        sca$parm.cons$log_R0[8],
        FIMS::logit(0.2, 1.0, 0.99),
        log(sca$parm.cons$rec_sigma[8])
      ),
      estimation_type = "constant"
    ),
    by = c("module_name", "label")
...
index_results <- data.frame(
  yr = styr:endyr,
  observed = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    FIMS::get_data(data_4_model) |>
      dplyr::filter(type == "index", fleet == "survey1") |>
      dplyr::pull(observed)
  ),
  fims.expected = c(rep(NA, length(styr:get_start_year(data_4_model)) - 1), report$index_expected[[3]]),
  bam.expected = sca$t.series$U.CVT.pr
) |>
  dplyr::filter(observed != -999.00)
fleet1_catch_results <- data.frame(
  yr = styr:endyr,
  observed = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    FIMS::get_data(data_4_model) |>
      dplyr::filter(type == "catch", fleet == "fleet1") |>
      dplyr::pull(observed)
  ),
  fims.expected = c(rep(NA, length(styr:get_start_year(data_4_model)) - 1), report$catch_expected[[1]]),
  bam.expected = sca$t.series$L.COM.pr
)
fleet2_catch_results <- data.frame(
  yr = styr:endyr,
  observed = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    FIMS::get_data(data_4_model) |>
      dplyr::filter(type == "catch", fleet == "fleet2") |>
      dplyr::pull(observed)
  ),
  fims.expected = c(rep(NA, length(styr:get_start_year(data_4_model)) - 1), report$catch_expected[[2]]),
...
fleet1_F_results <- data.frame(
  ...
  fims.F.fleet1 = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1), 
    dplyr::filter(FIMS::get_estimates(fit), label == "log_Fmort", module_id == 1) |>
      dplyr::pull(estimated) |>
      exp()
  ),
  bam.F.fleet1 = log(sca$t.series$F.COM)
)
fleet2_F_results <- data.frame(
  yr = styr:endyr,
  fims.F.fleet2 = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    dplyr::filter(FIMS::get_estimates(fit), label == "log_Fmort", module_id == 2) |>
      dplyr::pull(estimated) |>
      exp()
  ),
  bam.F.fleet2 = log(sca$t.series$F.REC)
)
...
fims.naa <- fims.naa[-NROW(fims.naa), ]
popn_results <- data.frame(
  yr = styr:endyr,
  fims.ssb = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    report$spawning_biomass[[1]][1:get_n_years(data_4_model)]
  ),
  fims.recruits = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    report$expected_recruitment[[1]][1:get_n_years(data_4_model)] / 1000
  ),
  fims.biomass = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    report$biomass[[1]][1:get_n_years(data_4_model)]
  ),
  fims.abundance = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    rowSums(fims.naa) / 1000
  ),
...
fims.fleet1.ncaa <- matrix(report$catch_numbers_at_age[[1]], ncol = n_ages, byrow = TRUE)
...
fims.fleet2.ncaa <- matrix(report$catch_numbers_at_age[[2]], ncol = n_ages, byrow = TRUE)
...
fims.survey.ncaa <- matrix(report$catch_numbers_at_age[[3]], ncol = n_ages, byrow = TRUE)
...
plot(fleet1_catch_results$yr, fleet1_catch_results$observed,
  ylim = c(0, max(fleet1_catch_results[, -1], na.rm = TRUE)),
  pch = 16, col = cols[1], ylab = "Fleet1 catch (mt)", xlab = ""
)
lines(fleet1_catch_results$yr, fleet1_catch_results$bam.expected, lwd = 3, col = cols[2])
lines(fleet1_catch_results$yr, fleet1_catch_results$fims.expected, lwd = 3, col = cols[4])

plot(fleet2_catch_results$yr, fleet2_catch_results$observed,
  ylim = c(0, max(fleet2_catch_results[, -1], na.rm = TRUE)),
  pch = 16, col = cols[1], ylab = "Fleet2 catch (mt)", xlab = ""
)
lines(fleet2_catch_results$yr, fleet2_catch_results$bam.expected, lwd = 3, col = cols[2])
lines(fleet2_catch_results$yr, fleet2_catch_results$fims.expected, lwd = 3, col = cols[4])
...
plot(fleet1_F_results$yr, fleet1_F_results$bam.F.fleet1,
  ylim = c(0, max(fleet1_F_results[, -1], na.rm = TRUE)),
...
plot(fleet2_F_results$yr, fleet2_F_results$bam.F.fleet2,
  ylim = c(0, max(fleet2_F_results[, -1], na.rm = TRUE)),
...
plot(popn_results$yr, popn_results$bam.ssb,
  ylim = c(0, max(popn_results[, c(2, 6)], na.rm = TRUE)),
  type = "l", lwd = 3, col = cols[2], ylab = "Spawning biomass (mt)", xlab = ""
)
...
plot(popn_results$yr, popn_results$bam.biomass,
  ylim = c(0, max(popn_results[, c(4, 8)], na.rm = TRUE)),
...
plot(popn_results$yr, popn_results$bam.recruits,
  ylim = c(0, max(popn_results[, c(3, 7)], na.rm = TRUE)),
...
plot(popn_results$yr, popn_results$bam.abundance,
  ylim = c(0, max(popn_results[, c(5, 9)], na.rm = TRUE)),
...
* Discards. I chose this assessment as a case study because it did not explicitly model discards, as I knew that capability was not yet available in FIMS. However, most assessments in the Southeast model dead discards.
