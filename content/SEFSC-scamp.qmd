...
# *_logSD is logged in create_default_DlnormDistribution()
# KFJ removed the "log() here
...
fleet1_landings_df <- data.frame(
  ...
  unit = "mt",
  # log(sqrt(log(1.0 + fleet1_landings_cv^2)))
...
res <- rbind(landings, index, agecomps, weight_at_age) |>
  dplyr::mutate(uncertainty = ifelse(value == -999, NA, uncertainty))
# TODO: See if the code needs to be corrected such that all values == -999
#       have a matching uncertainty of NA

data_4_model <- FIMS::FIMSFrame(res)

####################################################################################
# set up selectivities for fleets and survey
spp_fleet1_selectivity <- spp_fleet2_selectivity <- spp_survey1_selectivity <- fleet1 <- survey1 <- list(
  ...
  update_parameters(
    modified_parameters = list(
      ...
      recruitment = list(
        ...
        # FIX: transform to logit steepness
        BevertonHoltRecruitment.logit_steep.value = -log(1.0 - 0.99) + log(0.99 - 0.2), # Scamp used the null recruitment model -log(1.0 - 0.75) + log(0.75 - 0.2)
        # BevertonHoltRecruitment.logit_steep.value = 0.999, # Scamp used the null recruitment model -log(1.0 - 0.75) + log(0.75 - 0.2)
        DnormDistribution.log_sd.value = sca$parm.cons$rec_sigma[8], # did not log it
        BevertonHoltRecruitment.log_devs.estimated = TRUE, # KFJ added
        BevertonHoltRecruitment.log_devs.value = sca$t.series$logR.dev[-53] # KFJ cut off last b/c it must be length n_years - 1
      ),
      # Maturity
      ...
      population = list(
        ...
        Population.log_M.estimated = FALSE, # Added by KFJ, not sure what was done by Kyle
