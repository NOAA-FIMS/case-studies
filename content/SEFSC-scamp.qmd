...
data_4_model <- FIMS::FIMSFrame(
  ...
  #  |>
  # dplyr::filter(timing >= 1980 | is.na(timing))
)
```
...
default_parameters <- FIMS::setup_default_parameters(data_4_model) |>
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      fleet = "survey1",
      # timing = years,
      timing = seq(get_start_year(data_4_model), get_end_year(data_4_model)),
      label = "log_Fmort",
...
fleet1_F_results <- data.frame(
  ...
  fims.F.fleet1 = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1), 
    dplyr::filter(FIMS::get_estimates(fit), label == "log_Fmort", module_id == 1) |> dplyr::pull(estimated)
  ),
...
fleet2_F_results <- data.frame(
  ...
  fims.F.fleet2 = c(
    rep(NA, length(styr:get_start_year(data_4_model)) - 1),
    dplyr::filter(FIMS::get_estimates(fit), label == "log_Fmort", module_id == 2) |> dplyr::pull(estimated)
  ),
